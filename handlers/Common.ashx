...
public class Common : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string sqlfile = context.Request.MapPath("~/msql.xml");//在linux下面需要改成这样 在win下面也可以
        ...
        try
        {
            ...
            switch (pt.OperationCode)
            {
                ...
                case "XLS":
                    ...
                    break;
                case "query":
                    jsonText = getData(cp, dcd, sqlfile);
                    goto ExportExcel;
                default:
    ...
    private string getData(CommonParameter cp, DbCommonDDL dcd, string sqlfile)
    {
        string sql = string.Format(SQL.GetSQL(cp.ObjectName,sqlfile), cp.Values);
        cp.ObjectName = sql;

        JsonOutputParameters jsop= dcd.ExecuteSQL(cp);        
