...
agent.marketingUpd = (function () {
    ...
    formatPartner = '<tr data-id="{0}"><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td contenteditable="true">{5}</td><td contenteditable="true">{6}</td><td><input type="checkbox" {7} /></td></tr>',
    ...
    addPicker = function () {
        $('td[contenteditable="true"]').datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", format: "yyyy-mm-dd" });
    },
    addPeriod = function () {
    ...
    selectedPartner = function (p) {
        if (p) {
            if (o.dvStore.find('tr[data-id="' + p.id + '"]').size() == 0) {
                var tr = formatStr(formatPartner, [p.id, p.code, p.name, p.uname, p.mobile, gf.dateFormat(new Date(), 'yyyy-MM-dd'), gf.dateFormat(gf.addDay(new Date(), 8), 'yyyy-MM-dd'), 'checked="checked"']);
    ...
    updateStore = function (success) {
        ...
        o.dvStore.find('.report-results table tbody tr').each(function () {
            ...
            if (o.store[id]) {
                if (o.store[id][0] != tds.eq(4).text()
                    ...
                    || o.store[id][2] != (tds.eq(6).find(':checked').size() > 0 ? 'O' : 'C')) {
                    a.push(o.id);
                    a.push(id);
                    a.push(tds.eq(4).text());
                    a.push(tds.eq(5).text());
                    a.push(tds.eq(6).find(':checked').size() > 0 ? 'O' : 'C');
                    a.push(ur.user().id);
                    pu.push(a);
                }
            }
        ...
        if (pu.length) {
            gf.callProcBatch('up_member_updateMarketingStore', pu, function () {
                success ? success() : null;
            })
        } else {
            success ? success() : null;
    ...
    getStore = function (id) {
        var trs = [];
        gf.noPagination('up_member_getMarketingStore', [id, ur.user().agentId], function (a) {
            for (var i = 0, b = []; b = a[i]; i++) {
                b[5] = f.date(b[5]);
                b[6] = f.date(b[6]);
                o.store[b[0]] = [b[5], b[6], b[7]];
                b[7] = b[7].toUpperCase() == 'O' ? 'checked="checked"' : '';
...
agent.brand = (function () {
    var o = $.extend({}, gobj, gpagination, { dvId: 'dvBrand', ttbl: 'member.brand', fields: 'id,code,name,status,[desc]' }),
    ini = function () {
        if (!o.dv) {
            o.preIni();
...
agent.brandUpd = (function () {
    var o = $.extend({}, gobj, base.form, { dvId: 'dvBrandUpd', updProc: 'up_member_updateBrand', 
                    queryProc: 'up_member_getBrand',parent:agent.brand });
    return o;
})();
...
agent.funUpd = (function () {
    var o = $.extend({}, gobj, base.form, { dvId: 'dvFunctionUpd', updProc: 'up_base_updateFunction1',
        queryProc: 'up_base_getFunction1', parent: agent.fun
    });
    return o;
})();

agent.fun = (function () {
    var o = $.extend({}, gobj, gpagination, base.query, { dvId: 'dvFunction', updObj: agent.funUpd, ttbl: 'base.[function]', fields: 'id,code,name,status,[desc],[for],fileLoc' }),
    ini = function () {
        if (!o.dv) {
            o.ini();
        }
    ...
    o.fdata = function (a) {
        ...
        c.push(dom.td(dom.a(a[1], '#', 'data-id="' + a[0] + '"'))); //编号
        c.push(dom.td(a[2])); //名称
        c.push(dom.td(a[5])); //对象
        c.push(dom.td(a[6])); //文件
        c.push(dom.td(a[3] ? '有效' : '无效')); //状态
...
base.menuUpd = (function () {
    var o = $.extend({}, gobj, base.form, { dvId: 'dvMenuUpd', updProc: 'up_base_updateMenu',
        queryProc: 'up_base_getMenu', parent: base.menu
    });
    return o;
})();

base.menu = (function () {
    var o = $.extend({}, gobj, gpagination, base.query, { dvId: 'dvMenu', updObj: base.menuUpd, ttbl: 'base.menu_v', fields: 'id,code,name,status,[desc],fname,pname' }),
    ini = function () {
        if (!o.dv) {
            o.ini();
            o.updObj.ini();
            o.updObj.dv.find('.btnFunction').click(function () {
                var btn = $(this);
                base.functionLov.show(function (item) {
                    btn.prev().val(item.code);
                },'', btn.val())
            });
            o.updObj.dv.find('.btnParent').click(function () {
                var btn = $(this);
                base.menuLov.show(function (item) {
                    btn.prev().val(item.code);
                }, '', btn.val())
            });
        }
...
base.functionLov = (function () {
    var o = $.extend({}, gobj, base.lov, { queryProc: 'up_base_getFunctionLov', dvId: 'dvFunctionLov' });
    o.fdata = function (a) {
